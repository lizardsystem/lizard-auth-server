...
class UserProfileAdmin(admin.ModelAdmin):
    ...
    list_display = ['__unicode__', 'user', 'activation_email', 'is_activated']
    readonly_fields = ['created_at', 'updated_at', 'first_name', 'last_name', 'email', 'shortcut_urls']
    actions = ['send_new_activation_email']
    search_fields = ['user__first_name', 'user__last_name', 'user__email', 'activation_name', 'activation_email']

    def send_new_activation_email(self, request, queryset):
        for profile in queryset:
            if profile.is_activated:
                messages.error(request, 'UserProfile {} is already activated!'.format(profile), fail_silently=False)
            else:
                profile.send_new_activation_email()
    send_new_activation_email.short_description = ugettext_lazy('Send the activation email with a new key')
