...
class TestLoginRedirect(TestCase):
    def setUp(self):
        ...
        self.portal = factories.PortalF.create(
            sso_key=self.key, redirect_url=redirect, allowed_domain=allowed_domain
        )
    ...
    def authorize_and_check_redirect(self, domain, redirect):
        ...
        token = factories.TokenF.create(
            request_token=request_token, auth_token=auth_token, portal=self.portal
        )
    ...
    def test_login_redirect(self):
        ...
        self.authorize_and_check_redirect(
            "/this_is_fine.html", self.portal.redirect_url
        )
        self.authorize_and_check_redirect(
            "http://bad.com/wrong.aspx", self.portal.redirect_url
        )
        self.authorize_and_check_redirect(
            "http://very.custom.net/ok", "http://very.custom.net"
        )
        self.portal.allowed_domain = ""
        self.portal.save()
        self.authorize_and_check_redirect(
            "http://very.custom.net/nok", self.portal.redirect_url
        )
