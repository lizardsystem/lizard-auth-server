...
class RelevantPortalFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return (
            models.Portal.objects.filter(roles__isnull=False)
            .distinct()
            .values_list("id", "name")
        )
...
class RelevantBaseRoleFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return models.Role.objects.exclude(inheriting_roles__isnull=True).values_list(
            "id", "name"
        )
...
class RelevantOrganisationFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return (
            models.Organisation.objects.filter(organisation_roles__isnull=False)
            .distinct()
            .values_list("id", "name")
        )
